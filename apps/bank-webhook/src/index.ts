...
app.post("/hdfcWebhookMerchant", async (req, res) => {
  ...
  try {
    ...
    console.log(
      "Merchant exists?",
      await db.merchant.findUnique({ where: { id: Number(paymentInformation.merchantId) } })
    );

    console.log(
      "Merchant balance?",
      await db.merchantBalance.findUnique({ where: { merchantId: Number(paymentInformation.merchantId) } })
    );
    ...
    console.error(e);
    res.status(411).json({ message: "Error while processing merchant webhook" });
  }
});
